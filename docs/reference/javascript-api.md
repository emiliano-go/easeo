...
Register a generator for a schema type on the config, or replace the schema for
a single page with overrides.

```typescript
import { SchemaRegistry, buildSeoPayload } from "@easeo/core";

const registry = new SchemaRegistry();
registry.register("Article", (entity, cfg, canonical, title) => ({
  "@context": "https://schema.org",
  "@type": "PodcastEpisode",
  name: title,
}));

const config = { canonicalHost: "example.com", publicBaseUrl: "https://example.com", schemaRegistry: registry };
...
`getSchemaRegistry()` returns a native introspection helper (`has`,
`listTypes`) only; use the `SchemaRegistry` class to register generators.
